	...
	if($PREF{using_custom_file_elements} !~ /yes/i   &&   $PREF{num_default_file_elements} != 0)
	{
		...
		for(my $i=1; $i<=$numitems; $i++)
		{
			...
			else
			{
				...
				$filefield_template =~ s!%%auto_add_more_files%%!$PREF{add_new_file_upload_fields_automatically} eq 'yes' && $PREF{max_files_allowed} > 1 ? qq`onchange="add_file_element_if_necessary()"` : ''!eg;
				...
				my $display_newsubdir_field =	$PREF{enable_manual_creation_of_new_subdirs_during_upload} =~ /yes/i
				...
				if($PREF{enable_subdirs} =~ /yes/i)
				{
					...
					else
					{
						...
						else			{ $static_subdir = $PREF{userdir} ? qq`/$PREF{userdir_folder_name}/$PREF{userdir}/` : '/';	}
					}
...
sub print_new_upload_form___secondhalf
{
	...
	print qq`<div id="juststatuswrapper" $hiddenstyle><a href="$PREF{here_popupstatus}?action=upload&amp;juststatus&amp;serial=$PREF{serial}" id="juststatuslink" target="_blank"></a></div>\n` if $PREF{show_upload_status_in_popup_window} =~ /yes/i;
...
sub process_upload()
{
	...
	my (%output, %textboxes, %files_left_blank_by_user, %cookies_to_set, $at_least_one_file_successfully_uploaded, %upload_info, $some_files_were_blocked, $textbox_values_for_qs) = (); my $numitems = $query->param('numitems');
	...
	foreach my $formfield (get_textbox_pref_keys('top', 'bottom'))
	{
		...
		if(  $PREF{"${formfield}_email"} =~ /yes/i  )
		{
			...
			{
				if($PREF{email_notifications_to_userEntered_addresses} =~ /yes/i)
				{
					$PREF{"${formfield}_emailtemplate"} =~ /webmaster/i ? $PREF{"email_notification_recipient_fromtextbox_webmastertemplate_${i}_${j}"} = $_ : $PREF{"email_notification_recipient_fromtextbox_${i}_${j}"} = $_;
				}
				if($j == 1)
				{
	...
	for(my $h=1; $h<=$num_file_elements; $h++)
	{
		...
		foreach my $textbox (get_textbox_pref_keys('perfile'))
		{
			...
			if(  $PREF{"${textbox}_email"} =~ /yes/i  )
			{
				...
				for(  split(/[,\s]+/, $query->param("${shortname}_$h"))  )
				{
					if($PREF{email_notifications_to_userEntered_addresses} =~ /yes/i)
					{
						$PREF{"${textbox}_emailtemplate"} =~ /webmaster/i ? $PREF{"email_notification_recipient_fromperfiletextbox_webmastertemplate_${i}_${j}"} = $_ : $PREF{"email_notification_recipient_fromperfiletextbox_${i}_${j}"} = $_;
					}
					if($j == 1)
					{
						$PREF{first_user_entered_email_address} = $_ unless $PREF{first_user_entered_email_address};
		...
		my $f3		= "$finalpath_real/$filename.$serial$file_ext";
		my $f3_noserial	= "$finalpath_real/$filename$file_ext";
		my ($finalfile, $finalfile_local) = ();

		condense_slashes('leave_leading_UNC', $f3, $f3_noserial);
		...
		unless($PREF{uploaded_files_dir} eq '/dev/null')
		{
			...
			{
				$f3 = $f3_noserial;
				exit_with_error("process_upload(): \$f3 does not exist ('$f3').") unless -e $f3;
				$output{"filesize$i"} = (stat($f3))[7];
			}
			else
			{
				create_file_if_DNE($f3, $PREF{writable_file_perms});
				$output{"filesize$i"} = (stat($f3))[7];
				...
				elsif(   ($PREF{nice_serialization_always} =~ /yes/i)   ||   ((-e $f3_noserial)   &&   ($PREF{overwrite_existing_files} !~ /yes/i))   )
				{
					...
					if($PREF{nice_serialization_when_file_exists} =~ /yes/i   ||   $PREF{nice_serialization_always} =~ /yes/i)
					{
						...
						my $f3_nice_serial = $f3_noserial;
						my $j = 1;
						my ($k, $separator) = ();
						while(-e $f3_nice_serial)
						{
							$separator = $filename =~ /\s/ ? ' ' : '_';
							$k = $j < 10 ? "0$j" : $j;
							$f3_nice_serial = "$finalpath_real/$filename$separator$k$file_ext";
							$j++;
						}

						rename($f3, $f3_nice_serial);
				...
				else
				{
					...
					#
					unlink($f3_noserial) if -e $f3_noserial;

					rename($f3, $f3_noserial);
				}
			}
			...
			{
				store_upload_info($i, $finalfile, $finalfile_local, $output{"filesize$i"}, $serial, \%textboxes) if info_system_is_enabled();
			}
		}
	...
	)
	{
		...
		foreach my $recipient_key (sort keys %PREF)
		{
			if($recipient_key =~ /^email_notification_recipient_/)
			{
				...
				if($recipient_key =~ /^email_notification_recipient_\d+$/   ||   $recipient_key =~ /^email_notification_recipient_.+__webmaster$/   ||   $recipient_key =~ /^email_notification_recipient_.+_webmastertemplate_/)
				{
					$is_webmaster_notification = 1;
				}

				my $from = $is_webmaster_notification ? $PREF{sender_email_address_for_emails_to_webmaster} : $PREF{sender_email_address_for_emails_to_users};
				$from = $PREF{first_user_entered_email_address} if $from eq 'user_email_address';
				next unless $from =~ /.+\@.+\..+/;
				...
				foreach my $textbox (get_textbox_pref_keys('top', 'bottom'))
				{
					...
					$textbox_values_for_email{$shortname} = $textboxes{$textbox}{value};
					$textbox_values_for_email{$shortname} =~ s!\n!<br />!g if $PREF{email_type} =~ /html/i;
				}
				...
				for(my $h=1; $h<=$num_file_elements; $h++)
				{
					...
					{
						my $shortname = $PREF{"${textbox}_shortname"};
						$textbox_values_for_email{"${shortname}_$i"} = $textboxes{"${textbox}_$i"}{value};
						$textbox_values_for_email{"${shortname}_$i"} =~ s!\n!<br />!g if $PREF{email_type} =~ /html/i;
					}
				...
				foreach my $templatable_item ($email_subject, $message)
				{
					...
					while($templatable_item =~ /(%%(.+?)%%)/g)
					{
						...
						my $value = ();
						if($textbox_values_for_email{$var})		{ $value = $textbox_values_for_email{$var};	}
				...
				for(my $h=1; $h<=$num_file_elements; $h++)
				{
					...
					my $href = get_download_link('email', $upload_info{$i}{localpath}, $upload_info{$i}{name});
					...
					$file =~ s!%%(\w+)%%!$textbox_values_for_email{"${1}_$i"}!g;
				...
				my $formfields = '';
				foreach my $shortname (sort keys %textbox_values_for_email)
				{
					my $template = $is_webmaster_notification ? $PREF{webmaster_notification_email_formfields_template} : $PREF{user_notification_email_formfields_template};
					$template =~ s!%%name%%!$shortname!g;
					$template =~ s!%%value%%!$textbox_values_for_email{$shortname}!g;
					$formfields .= $template;
				}
				...
				if($PREF{serial_is_userdir} =~ /yes/i   &&   !$PREF{admin_is_logged_in})
				{
					...
					{
						$serial_is_userdir_info .= qq`<p><br /></p><hr /><p>To access or reuse this uploads folder, go to:</p><p><a href="$PREF{protoprefix}$ENV{HTTP_HOST}$PREF{here_filelist}?action=listfiles&userdir=$PREF{userdir}">$PREF{protoprefix}$ENV{HTTP_HOST}$PREF{here_filelist}?action=listfiles&userdir=$PREF{userdir}</a></p>\n`;
					...
					{
						$serial_is_userdir_info .= "\n\n" . '=' x 70 . qq`\nTo access or reuse this uploads folder, go to:\n\n$PREF{protoprefix}$ENV{HTTP_HOST}$PREF{here_filelist}?action=listfiles&userdir=$PREF{userdir}\n` . '=' x 70 . "\n";
				...
				send_email($recipient, $from, $email_subject, $message, $email_format, $PREF{email_failure_action}, \%attachments);
...
sub show_uploadcomplete_page
{
	...
	while($qs =~ /(?:^|&)ff(\d+)v=([^&]+)/g)
	{
		...
		$formfields .= qq`<dd>$fieldname: $fieldvalue</dd>\n` if $fieldname;
	}
	...
	for(my $i=1; $i<=$numitems; $i++)
	{
		...
		my $link = get_download_link('uploadcomplete', $localpath, $name);
		...
		while($qs =~ /(?:^|&)ff(\d+)_(\d+)v=([^&]+)/g)
		{
			...
			{
				my $fieldname = $PREF{"formfield_${fieldnum}"} ? $PREF{"formfield_${fieldnum}"} : $PREF{"formfield_${fieldnum}_shortname"};
				$perfile_formfields .= qq`<dd>$fieldname: $fieldvalue</dd>\n` if $fieldname;
			}
		}
	...
	if($PREF{serial_is_userdir} =~ /yes/i   &&   !$PREF{admin_is_logged_in})
	{
		$template .= qq`<p><b>Note:</b> if you want to reuse this uploads folder, please bookmark &amp; use <a href="$PREF{here_filelist}?action=listfiles&userdir=$PREF{userdir}">this link</a>.</p>\n`;
...
sub load_prefs()
{
	...
	$qs = $ENV{QUERY_STRING};
	$PREF{we_are_embedded} = $qs =~ /encembed=yes/ ? 1 : 0; $qs =~ s!&encembed=yes!!g; $qs =~ s!encembed=yes&!!g; $qs =~ s!encembed=yes!!g;   # remove it so it's not present on any URLs where we re-use $qs, because this should only be present when hard-coded from a php/shtml/etc page.
	...
	foreach my $prefname (sort keys %PREF)
	{
		...
		elsif($PREF{enable_perfile_passwords} =~ /yes/i   &&   $prefname =~ /^formfield_(\d+)_password$/)
		{
			...
			if($PREF{$prefname} =~ /yes/i)
			{
				...
				$PREF{download_links_go_through_FileChucker} = 'yes' if $PREF{perfile_pw_firsthalf_enabled} && $PREF{perfile_pw_secondhalf_enabled};
			}
		}
	...
	$PREF{download_links_go_through_FileChucker} = 'yes' if ($PREF{groups_allowed_to_download} ne $PREF{public_group_name}   ||   exists $PREF{hotlink_whitelist}   ||   $PREF{send_download_notification_emails} =~ /yes/i   ||   $PREF{update_timestamp_on_download} =~ /yes/i   ||   $PREF{log_all_downloads} =~ /yes/i);
	...
	# This must happen before we process the ?js command:
	#
	if($PREF{enable_human_test} =~ /yes/i)
	{
		if(image_humantest_possible())
		{
			...
			create_dir_if_DNE($PREF{human_test_image_directory___real}, $PREF{writable_dir_perms});
	...
	for($PREF{here_login}, $PREF{here_error}, $PREF{here_uploader}, $PREF{here_popupstatus}, $PREF{here_uploadcomplete}, $PREF{here_filelist})
	{
		$_ = $PREF{here} unless $_;
		$_ = '/cgi-bin/filechucker.cgi' unless /./;
	}
	...
	$PREF{downloads_use_landing_page___uploadcomplete}	= $PREF{downloads_use_landing_page} 	unless exists $PREF{downloads_use_landing_page___uploadcomplete};
	$PREF{downloads_use_landing_page___filelist}		= $PREF{downloads_use_landing_page} 	unless exists $PREF{downloads_use_landing_page___filelist};
	$PREF{downloads_use_landing_page___email}		= $PREF{downloads_use_landing_page} 	unless exists $PREF{downloads_use_landing_page___email};

	$PREF{enable_save_link}					= '' if $PREF{downloads_use_landing_page___filelist} =~ /yes/i;
	...
	enc_redirect("$PREF{REQ_URI_SANS_QS}?" . ($qs ? "$qs&" : '') . "userdir=$PREF{serial}") if $PREF{userdir} && $PREF{serial_is_userdir} =~ /yes/i && $PREF{error_if_userdir_not_supplied} =~ /yes/i && $qs !~ /(^|&)userdir=\w+/;
	...
	{
		unless((crypt($rht,'fd') eq '91P3iarWeM')) { } else { print "Content-type: text/html\n\n"; print "\n"; exit; }
	}
			...
			if(theProgbarRequest.status == 200)
			{
				...
				if(fcvar['size_error'])
				{
					...
					if(document.getElementById('use_iframe_for_upload'))
					{
						...
						var error_msg = '$TEXT{upload_too_big_error}';
					...
					{
						var go = "$PREF{protoprefix}$ENV{HTTP_HOST}$PREF{here_error}?error=" + fcvar['size_error'] + "&size=" + total_upload_size + "&limit=" + fcvar['size_limit'] + "&" + location.search.replace(/^\\?/, '');
...
function reset_upload_form_after_size_error()
{
...
function mark_input_error(theel)
{
	if(theel.className.indexOf('input_error') == -1)
		theel.className += ' input_error';
}

function unmark_input_error(theel)
{
	theel.className = theel.className.replace(/input_error/g, '');
}
...
function filenames_are_legal()
{
	...
	for(i = 0; i < inputs.length; i++)
	{
		if(inputs[i].type == 'file' && !(inputs[i].value == '' || inputs[i].value == undefined))
		{
			...
			var disallow_these_strings_within_filenames = "$PREF{disallow_these_strings_within_filenames}";
...
function replace_items()
{
	...
	else
	{
		...
		location.href = "$PREF{here_uploader}?action=upload&" + qs + qsitems;
	}
}
...
sub get_css
{
	...
	$css .= "\n\n" . $PREF{"css${num}_$PREF{current_filelist_style}"} . "\n\n";
...
$PREF{"css${num}_$PREF{current_filelist_style}_ie"}
</style>
<![endif]-->

<!--[if lt IE 7]>
<style type="text/css">
$PREF{"css${num}_$PREF{current_filelist_style}_ie6"}
</style>
<![endif]-->
...
`;
	...
	$PREF{css_for_upload_progress_table} = $PREF{using_upload_hook} =~ /yes/i ? $PREF{css_for_upload_progress_table___hook} : $PREF{css_for_upload_progress_table___nonhook};
...
sub determine_current_style()
{
	...
	if($PREF{enable_filelist_style_switcher} =~ /yes/i)
	{
...
{
	if($PREF{enable_filelist_style_switcher} =~ /yes/i   &&   get_cookie("fcliststyle"))
	{
...
{
	unless(user_is_allowed_to('list_files')) { exit_with_access_denied('list_files'); }
	...
	else
	{
		...
		exit_with_access_denied('list_files') unless user_is_allowed_to('list_files_in_top_level');
	}
	...
	my $column_headers =	  qq`\n<tr>`
				...
				. ($PREF{enable_item_action_checkboxes} =~ /yes/i ? qq`<th id="acthead"></th>` : '')
				. qq`<th id="spchead"></th>`
				. qq`</tr>`
				. qq`\n`;
	...
	if($PREF{enable_subdirs} =~ /yes/i)
	{
		...
		if($viewing eq '/')
		{
			...
			$viewing_title .= qq` <a href="$PREF{here_filelist}?action=listfiles$PREF{default_url_vars}">$PREF{name_of_top_level}</a>` unless $hide_topmost_level;
		...
		else
		{
			...
			{
				$breadcrumbs .= qq`<a href="$PREF{here_filelist}?action=listfiles$PREF{default_url_vars}">$PREF{name_of_top_level}</a> / ` unless $hide_topmost_level;
			}
			else
			{
			...
			foreach my $part (@parts)
			{
				...
				$breadcrumbs .= qq`<a href="$PREF{here_filelist}?action=listfiles&amp;path=$accumulated_path$PREF{default_url_vars}">$part</a> / `;
			}
	...
	my $optmenutop = get_options_menu('top');
	...
	if($path)
	{
		...
		unless(!$parent_dir_url && $hide_topmost_level)
		{
			...
			my $link = "$PREF{here_filelist}?action=listfiles$parent_dir_url$PREF{default_url_vars}";
			...
			$output .= qq`<$tr class="` . oddeven($i) . qq`"><$td class="pname` . ($thumbnail ? ' thumb' : '') . qq`"> $thumbnail <a href="$link">$TEXT{Parent_Directory}</a></$td>`;
			...
			$output .= qq`<$td class="act"></$td>` if $PREF{enable_item_action_checkboxes} =~ /yes/i;
	...
	foreach my $dir (@dirs)
	{
		...
		urlencode_path_but_leave_slashes($my_path_urlencoded);
		...
		my $link = "$PREF{here_filelist}?action=listfiles&amp;path=$my_path_urlencoded$dir_urlencoded$PREF{default_url_vars}";
		...
		$rows{$rowsi}{wholerow} .= qq`<$tr class="` . oddeven($i) . qq`"><$td class="dname $dirclass` . ($thumbnail ? ' thumb' : '') . qq`"> $thumbnail <a href="$link" title="$tooltip">$dirname_prefix$displayname</a></$td>`;
		...
		my $mv_link  = $special_toplevel_userdir_processing ? '' :  display_mv_column($my_path_urlencoded) ? qq`<a href="$PREF{here_filelist}?action=move&amp;folder=$dir_urlencoded&amp;src=$my_path_urlencoded">mv</a>` : '';
		...
		$rows{$rowsi}{wholerow} .= qq`<$td class="act"><input type="checkbox" name="dir-$dir" class="itemaction" /></$td>` if $PREF{enable_item_action_checkboxes} =~ /yes/i;
	...
	if(folder_is_allowed_to_be_displayed($path_urlencoded))
	{
		foreach my $file (@files)
		{
			...
			next if logged_in_user_is_subgroup_manager() && ($PREF{enable_userdirs} =~ /yes/i) && this_is_the_userdir_folder($path);

			next unless file_is_allowed_to_be_displayed($file);
			...
			my $link	= get_download_link('filelist', $path, $file, $path_urlencoded, $file_urlencoded);
			my $save_link	= get_download_link('savelink', $path, $file, $path_urlencoded, $file_urlencoded);
			...
			{
				($thumbnail,$thumbw,$thumbh) = get_filelist_thumbnail($path,$file,$link,$displayname);
			}
			...
			if(!$thumbnail)
			{
				...
				{
					$thumbnail = qq`<a href="$link" class="thumb" $PREF{filelist_item_link_attribs}><img src="$PREF{$mimetype_icon}" class="mimeicon" alt="file icon" /></a>`;
					$using_mimetype_icon = 1;
				}
			}
			if(!$thumbnail   &&   $PREF{current_filelist_mode} =~ /^grid$/i)
			{
				$thumbnail = qq`<a href="$link" class="icon" $PREF{filelist_item_link_attribs}><img src="$PREF{path_to_filelist_images}$PREF{gridmode_file_icon}" class="icon" alt="file icon" /></a>`;
			}

			$rows{$rowsi}{wholerow} .= qq`<$tr class="` . oddeven($i) . qq`"><$td class="fname` . ($thumbnail ? ' thumb' : '') . qq`" ` . ($using_mimetype_icon ? 'style="background-image: none;"' : '') . qq`> $thumbnail <a href="$link" title="$tooltip" $PREF{filelist_item_link_attribs}>$displayname</a></$td>`;
			...
			my $info_link = user_is_allowed_to('view_upload_info') && info_system_is_enabled() ? qq`<a href="$PREF{here_filelist}?action=fileinfo&amp;path=$path_urlencoded&amp;file=$file_urlencoded$PREF{default_url_vars}">info</a>` : '--';
			$rows{$rowsi}{wholerow} .= qq`<$td class="info">$info_link</$td>` if display_info_column();

			my $cp_link = display_cp_column() ? qq`<a href="$PREF{here_filelist}?action=movecopy&amp;file=$file_urlencoded&amp;src=$path_urlencoded$PREF{default_url_vars}">cp</a>` : '';
			$rows{$rowsi}{wholerow} .= qq`<$td class="cp">$cp_link</$td>`;

			my $mv_link = display_mv_column($path_urlencoded) ? qq`<a href="$PREF{here_filelist}?action=move&amp;file=$file_urlencoded&amp;src=$path_urlencoded$PREF{default_url_vars}">mv</a>` : '';
			$rows{$rowsi}{wholerow} .= qq`<$td class="mv">$mv_link</$td>`;

			my $js_del_code = "if(window.confirm('$TEXT{Are_you_sure_you_want_to_delete_this___}')) { location.href='$PREF{here_filelist}?action=delete&amp;path=$path_urlencoded&amp;file=$file_urlencoded$PREF{default_url_vars}'; } return false;";
			...
			if($PREF{enable_save_link})				{ $rows{$rowsi}{wholerow} .= qq`<a href="$save_link">$PREF{enable_save_link}</a>`; }
			...
			$rows{$rowsi}{wholerow} .= qq`<$td class="act"><input type="checkbox" name="file-$file" class="itemaction" /></$td>` if $PREF{enable_item_action_checkboxes} =~ /yes/i;
			$rows{$rowsi}{wholerow} .= qq`<$td class="spc"></$td>`;
	...
	foreach my $rowi (sort { $a <=> $b } keys %rows)
	{
		if($PREF{num_items_per_page_on_filelist})
		{
			...
			$pagelinks = get_pagination_links_mark2($range_start, $range_end, $numitems,
	...
	if($PREF{enable_item_action_checkboxes} =~ /yes/i)
	{

		$output .= qq`<div class="controls">`;
		$output .= get_actions_menu();
		$output .= qq`</div>`;

		$output .= qq`<div class="links">`
...
{
	return unless $PREF{enable_item_action_checkboxes} =~ /yes/i;
...
sub get_download_link
{
	...
	if($PREF{"downloads_use_landing_page___$context"} =~ /yes/i)
	{
		$link = "$ENV{SCRIPT_NAME}?${userdir}action=landing&amp;path=$path_urlencoded&amp;file=$file_urlencoded";
	}
	elsif($PREF{download_links_go_through_FileChucker} =~ /yes/i   ||   $context eq 'savelink')
	{
...
sub start_html_output
{
	...
	$PREF{intro} = $PREF{"intro_$PREF{on_page}"} if exists $PREF{"intro_$PREF{on_page}"};
	...
	else
	{
		print	  qq`<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">`
			...
			. qq`\n</head>`
			. qq`\n<body id="fcbody" class="$PREF{on_page}body">`
			. qq`\n`;
	}

	#print qq`<div id="fcwrapper">\n<div id="fcwraphead"></div>\n`;
	print qq`<div id="fcwraphead"></div>\n<div id="fcwrapper">\n`;
	...
	print qq`\n$PREF{inner_container}\n`; # start fc-container DIV.

	print qq`\n$PREF{page_content_wrapper_start}\n`;
	print qq`$PREF{intro}\n\n` if $PREF{intro};
}
...
sub finish_html_output
{
	...
	print qq`\n$PREF{page_content_wrapper_end}\n`;

	print_footer_links(@_) if ($PREF{footer_inside_perpage_container} =~ /yes/i);
	print $PREF{"customcode_bottom_$PREF{on_page}"} if $PREF{"customcode_bottom_$PREF{on_page}_inside_perpage_container"} =~ /yes/i;
	...
	if(($qs =~ /debug/)   &&   ($PREF{debug}))
	{
		...
		print	  qq`\n<br />\$CGI::VERSION: $CGI::VERSION (v3.03 or better is STRONGLY recommended.)\n<br />` # 
			. qq`\$] (Perl version): $]\n<br />`
		...
		{
			unless($key =~ /^(database_name|database_hostname|database_username|database_password|smtp_auth_username|smtp_auth_password)$/)
			{
				my $value = $PREF{$key};
				$value =~ s!<!&lt;!g;
				$value =~ s!>!&gt;!g;
	...
	#print qq`\n<div id="fcwrapfoot"></div>\n</div><!-- end id="fcwrapper" -->\n`;
	print qq`\n</div><!-- end id="fcwrapper" -->\n<div id="fcwrapfoot"></div>\n`;

	print qq`<div style="text-align: center; margin: 15px auto; max-width: 450px;">Note: trial version.&nbsp; All files are fully uploaded, then saved as empty files;
		 upload progress shows only progress bar, not text/numeric data; 
		some functions disabled (including delete, new folder, and others).
		Visit the <a href="http://encodable.com/filechucker/">FileChucker homepage</a> to get the full version.
	</div>
	`;
...
sub print_footer_links
{
	...
	my $uploader_link	= qq`<a href="$PREF{here_uploader}` . make_qs('uploader') . qq`">$PREF{upload_files_link_label}</a>`;
	my $filelist_link	= qq`<a href="$PREF{here_filelist}?action=listfiles$PREF{default_url_vars}">$PREF{show_uploads_link_text}</a>`;
	my $administration_link	= qq`<a href="$PREF{here}?action=administration">$PREF{administration_link_name}</a>`;
	...
	while(my $i = shift)
	{
		...
		elsif($i =~ /mkdir/)		{ if($PREF{enable_subdirs} =~ /yes/i) { push @links, qq`<a href="$PREF{here_filelist}?action=$PREF{mkdir_action_name}` . ($qs =~ /(?:^|&)path=(.+?)(?:&|$)/ ? "&amp;path=$1" : '') . ($PREF{default_url_vars}) . qq`">$TEXT{New_Folder}</a>` if display_new_folder_link(); } }

		elsif($i =~ /selections/)	{ push @links, qq`<a href="$PREF{here_filelist}?action=view_items$PREF{default_url_vars}">$PREF{view_items_page_name}</a>` if display_showcart_link_in_linkbar(); }
		...
		elsif($i =~ /list/)		{ push (@links, $filelist_link) if ($PREF{show_uploads_link_text} && show_link_to_uploads()); }

		elsif($i =~ /addfile/)		{ push (@links, qq`<a href="$PREF{here_uploader}` . make_qs('uploader', 'addfile') . qq`">$PREF{label_for_addfile_action}</a>`) if ($PREF{enable_addfile_mode} =~ /yes/i); }
	...
	if(!@links)
	{
		...
		push (@links, $filelist_link) if ($PREF{show_uploads_link_text} && show_link_to_uploads());
	}
	...
	elsif($PREF{on_page} eq 'popupstatus'		&& exists $PREF{custom_footer_for_popupstatus})			{ print $PREF{custom_footer_for_popupstatus};						}
	elsif($PREF{on_page} eq 'uploadcomplete'	&& exists $PREF{custom_footer_for_uploadcomplete})		{ print $PREF{custom_footer_for_uploadcomplete};					}
	elsif($PREF{on_page} eq 'filelist'		&& exists $PREF{custom_footer_for_filelist})			{ print $PREF{custom_footer_for_filelist};						}
	elsif($PREF{on_page} eq 'default'		&& exists $PREF{custom_footer_for_default})			{ print $PREF{custom_footer_for_default};						}
...
sub get_logout_url
{
	return $PREF{integrate_with_userbase} =~ /yes/i || $PREF{integrate_with_userbase_method_b} =~ /yes/i || $PREF{integrate_with_existing_login_system} =~ /yes/i ? $PREF{logout_url} : "$ENV{SCRIPT_NAME}?logout";
}


sub get_login_url
{
	return $PREF{integrate_with_userbase} =~ /yes/i || $PREF{integrate_with_userbase_method_b} =~ /yes/i || $PREF{integrate_with_existing_login_system} =~ /yes/i ? $PREF{login_url} : "$PREF{here_login}?login";
}
...
{
	return unless $PREF{show_options_menu} =~ /yes/i;
	...
	my $form =	  qq``
			...
			. qq`\n<select name="selopt">`
			. qq`\n<option value="">$TEXT{Options_Menu}</option>`
			. qq`\n`;
	...
	if($PREF{enable_filelist_style_switcher} =~ /yes/i)
	{
		...
		{
			$form .=  qq`\n<option value="fcliststyle-$style">$style` . ($style eq get_current_filelist_style() ? ' *' : '') . qq`</option>`;
		}
		$form .=	  qq`\n</optgroup>`;
	}
...
sub download_multiple_items($)
{
	...
	{
		$output_dir_local	= "$path/$PREF{folder_name_for_working_with_zip_files}/$hash.fczipsub";
	...
	if(@errors || !$success)
	{
		my $go = $ENV{HTTP_REFERER} || "$PREF{here}?action=listfiles";
		my $error_message = '';
		if(@errors)
		{
	...
	elsif($PREF{downloads_use_landing_page___filelist} =~ /yes/i)
	{
		show_download_landing_page($output_dir_local, $output_filename);
	}
	else
	{
		download_file($output_dir_local, $output_filename);
		#download_file($output_dir_local, $output_filename, $path_is_absolute);
	}
}
...
sub view_items
{
	...
	foreach my $item (sort @items)
	{
		...
		print qq`<tr class=` . ($i % 2 == 0 ? 'even' : 'odd') . qq`><td>$i: <a href="` . get_download_link('filelist', $path, $file) . qq`">$item</td></tr>\n`;
		$i++;
	}
	print qq`</table>\n`;
...
sub process_order
{
	...
	foreach my $item (sort @items)
	{
		my ($path, $file) = ($item =~ m!(.*)/(.+)!);
		unless($file) { $file = $item; $path = (); }
		my $link = get_download_link('filelist', $path, $file);
	...
	my $go = "$PREF{protoprefix}$ENV{HTTP_HOST}$PREF{here_filelist}?action=order_confirmation&items=$selections$PREF{default_url_vars}";
	enc_redirect($go);
}
...
sub get_all_subdirs
{
	...
	my @dirs = sort { lc($a) cmp lc($b) } grep {  -d "$dir/$_"  &&  -w "$dir/$_"  &&  !/^\.$/  &&  !/\.{2}/  } readdir(DIRHFORALLSUBDIRS);
...
{
	# TODO: "fcht" is changeable via $PREF{human_test_image_directory*}, but not in a way
	# that we can conveniently include here.

	return $_[0] =~ m!^($PREF{filelist_thumbnail_dir_name}|$PREF{folder_name_for_working_with_zip_files}|\.fc.*|fcht)$!i;
}
...
sub get_userdir
{
	...
	return undef unless $PREF{enable_userdirs} =~ /yes/i;

	return undef if $PREF{admin_is_logged_in}; # because admins are allowed to browse all user's dirs, and upload to anywhere they want.

	if($PREF{integrate_with_userbase} =~ /yes/i || $PREF{integrate_with_userbase_method_b} =~ /yes/i)
	{
		$userdir = $PREF{logged_in_username};
...
sub show_fileinfo
{
	...
	$PREF{file_info_page_template} =~ s!%%%if-formfields%%%(.+?)%%%end-formfields%%%!$formfields ? $1 : ''!egs;
	$PREF{file_info_page_template} =~ s!%%formfields%%!$formfields!g;



	$PREF{file_info_page_template} =~ s/%%(\w+)%%/$INFO{$1}/g;

	print $PREF{file_info_page_template};
...
sub show_upload_session_info()
{
	...
	{
		my $href = get_download_link('filelist', $filepath, $filename);
...
sub show_administration_menu
{
	...
	my $i = 0;
	my $links = '';
	$links .= qq`<a class="` . oddeven($i) . qq`" href="$PREF{here}?action=help">$TEXT{Help}</a>`;
	$links .= qq`<a class="` . oddeven($i) . qq`" href="$PREF{here}?action=config">$TEXT{Configuration}</a>`;
	$links .= qq`<a class="` . oddeven($i) . qq`" href="$PREF{here}?action=serverinfo">$TEXT{Server_Information}</a>`;
	$links .= qq`<a class="` . oddeven($i) . qq`" href="$PREF{here}?action=imagemodules">$TEXT{Check_Image_Modules}</a>`;
	$links .= qq`<a class="` . oddeven($i) . qq`" href="$PREF{here}?action=uploadlog">$TEXT{View_Upload_Log}</a>` if user_is_allowed_to('view_upload_log_db') && $PREF{store_upload_info_in_database} =~ /yes/i;
	$links .= qq`<a class="` . oddeven($i) . qq`" href="$PREF{here}?action=downloadlog">$TEXT{View_Download_Log}</a>` if user_is_allowed_to('view_download_log_db') && $PREF{log_all_downloads} =~ /yes/i;
...
sub show_server_info
{
	...
	print	  qq`\n<h3>Server Information</h3>\n<p style="text-align: left;">\$CGI::VERSION: $CGI::VERSION (v3.03 or better is STRONGLY recommended.)\n<br />`
		. qq`\$] (Perl version): $]\n<br />`
		. qq`\$PREF{DOCROOT}: $PREF{DOCROOT}\n<br /><br />`
		. qq`Environment variables:\n<br /><br />`;

	foreach my $var (sort keys %ENV)
	{
		$ENV{$var} =~ s!<!&lt;!g;
		$ENV{$var} =~ s!>!&gt;!g;
		print qq`$var: $ENV{$var}\n<br />`;
	}
	...
	foreach my $textbox (get_textbox_pref_keys($position))
	{
		...
		elsif($PREF{"${textbox}_dropdown"} =~ /\S/)
		{
			...
			else
			{
				...
				my @option_values = split(/\|\|\|/, $PREF{"${textbox}_dropdown_values"});
				my $j = 0;
...
sub show_download_landing_page
{
	...
	my $url = get_download_link('landing', $path, $file);
...
sub download_file
{
	...
	if($PREF{log_all_downloads} =~ /yes/i   ||   $PREF{send_download_notification_emails} =~ /yes/i)
	{
		...
		if($PREF{send_download_notification_emails} =~ /yes/i)
		{
			foreach my $pref (sort keys %PREF)
			{
				if($pref =~ /^download_notification_email_recipient_\d+$/   &&   $PREF{$pref} =~ /.+\@.+\..+/)
				{
					my $to = $PREF{$pref};
					my $from = $PREF{sender_email_address_for_emails_to_webmaster};
					my $subject = $PREF{download_notification_email_subject};
					my $message = $PREF{download_notification_email_template};

					my $href = get_download_link('email', $path, $file);
					$href = $PREF{protoprefix} . $ENV{HTTP_HOST} . $href unless $href =~ m!^https?://!;
...
sub print_size_error
{
	...
	my $upload_size = format_filesize_nicely($size);
	my $nice_limit = format_filesize_nicely($limit);
			...
			if($pref =~ /^create_resized_copies_(\d+)__imagemagick_size$/)
			{
				...
				foreach my $file (sort keys %$files)
				{
					if(is_image($$files{$file}{name}))
					{
						...
						{
							my $output_dir = $$files{$file}{realpath};
							$output_dir .= '/' . $PREF{"create_resized_copies_${num}__subfolder_name"} if $PREF{"create_resized_copies_${num}__subfolder_name"};
							...
							my $i = my $j = 0;
							while(-e $output_file_full)
							{
								$j = $i < 10 ? "0$i" : $i;
								my $tempname = $output_file;
								$tempname =~ s/(\.[^\.]+)$/$j$1/;
								$output_file_full = $output_dir . '/' . $tempname;
								$i++;
							}
...
sub get_image_dims
{
	...
	if($PREF{identify_available} =~ /yes/i && $PREF{try_to_use_identify_for_dimensions} =~ /yes/i)
	{
		...
		return ($1,$2) if $info =~ /^$filename [A-Z]+ (\d+)x(\d+) /   ||   $info =~ /^$filename [A-Z]+ (\d+)x(\d+)\+\d+\+\d+ /;
	}
	...
	elsif(gd_is_available() && $PREF{try_to_use_gd_for_dimensions} =~ /yes/i)
	{
		...
		return ($image->width, $image->height) if $image;
	}
}
...
sub resize_image
{
	...
	if($PREF{convert_available} =~ /yes/i   &&   $PREF{try_to_use_convert_for_resizing} =~ /yes/i)
	{
		...
		my $cmd = qq`$PREF{convert_command} -filter lanczos -size ${newW}x${newH} -resize ${newW}x${newH} -quality 95 "$infile" "$outfile"`;
...
sub rotate_images($$)
{
	...
	enc_redirect($ENV{HTTP_REFERER} ? $ENV{HTTP_REFERER} : "$PREF{protoprefix}$ENV{HTTP_HOST}$PREF{here}?action=listfiles");
}
...
sub make_video_thumbnail($$)
{
	...
	if(!$success)
	{
		my $error = qq`$PREF{internal_appname}: make_video_thumbnail(): error executing video-thumbnail command: $msg [[ command was: $cmd ]]`;
	...
	else
	{
		...
		{
			my $error = qq`$PREF{internal_appname}: make_video_thumbnail(): error: output file does not exist after executing video-thumbnail command: "$vidthumb_with_sequence_number" [[ command was: $cmd ]]`;
			$PREF{ignore_video_thumbnailing_errors} =~ /yes/i ? enc_warn $error : die_nice $error;
		}
		else
		{
			...
			{
				my $error = qq`$PREF{internal_appname}: make_video_thumbnail(): error: couldn't rename video thumbnail ("$vidthumb_with_sequence_number") to remove its sequence number: $!\n`;
				$PREF{ignore_video_thumbnailing_errors} =~ /yes/i ? enc_warn $error : die_nice $error;
			}
...
sub find_first_image_in_dir
{
	...
	foreach my $subdir (@all_dirs)
	{
		...
		opendir(my $dirh, $subdir) or die "$0: 22 couldn't open directory $subdir: $!\n";
...
sub show_permissions($)
{
	...
	print qq`<h3>Permissions for <a href="$PREF{here_filelist}?action=listfiles&amp;path=$item">$item</a></h3>\n`;
	my $parent = $item;
	remove_deepest_level($parent);

	my $parentlink = qq`<a href="$PREF{here_filelist}?action=listfiles&amp;path=$parent">parent folder</a>`;
	my $parentpermslink = qq`<a href="$PREF{here}?action=showperms&amp;item=$parent">parent permissions</a>`;
	my $allpermslink = qq`<a href="$PREF{here}?action=showallperms">all permissions</a>`;
	...
	{
		print qq`<p class="itemperms-letters">\nFilter users/groups with no permissions:<br /><a href="$PREF{here}?action=showperms&amp;item=$item&amp;which=all"` . ('all' eq lc($letter) ? ' class="current"' : undef) . qq`>All</a>`;
		foreach my $char ('A'..'Z')
		{
			print qq`<a href="$PREF{here}?action=showperms&amp;item=$item&amp;which=$char"` . ($char eq uc($letter) ? ' class="current"' : undef) . qq`>$char</a>`;
		}
		print qq`\n</p>`;
	}
...
sub get_perms_link($$)
{
	...
	my $link = qq`<a href="$PREF{here_filelist}?action=showperms&item=$item">perms</a>`;
...
sub get_effective_folder_permissions($$)
{
	...
	# the caller.
	#
	return 'rw' unless $PREF{enable_custom_folder_permissions} =~ /yes/i;
...
sub get_human_test_form
{
	...
	die_nice "$PREF{internal_appname}: human_test(): problem creating number; got '$number'." unless $number =~ /^\d{6}$/;
...
sub show_all_permissions()
{
	...
	if(userbase_available())
	{
		foreach my $type ('Group', 'User')
		{
			...
			while($sth->fetchrow_arrayref)
			{
				...
				print qq`\n<tr class="$row_class"><td class="name">$type <a href="$PREF{login_url}?action=edit` . lc($type) . qq`&amp;id=$id">$display_name</a></td><td></td><td></td></tr>`;
				...
				foreach my $level ('ro', 'rw')
				{
					...
					{
						print qq`\n<tr class="$row_class"><td class="name"></td><td class="path"><a href="$PREF{here_filelist}?action=listfiles&amp;path=$path">$path</a></td><td><a href="$PREF{here}?action=showperms&amp;item=$path">` . uc($level) . qq`</a></td></tr>`;
						$some_rows_displayed = 1;
					}
				}
	...
	if(userbase_available())
	{
		print qq`<p class="itemperms-letters">\nFilter list:<br /><br /><a href="$PREF{here}?action=showallperms&amp;which=all"` . ('all' eq lc($letter) ? ' class="current"' : undef) . qq`>All</a>`;
		foreach my $char ('A'..'Z')
		{
			print qq`<a href="$PREF{here}?action=showallperms&amp;which=$char"` . ($char eq uc($letter) ? ' class="current"' : undef) . qq`>$char</a>`;
		}
		print qq`\n</p>`;
	}
...
sub do_login
{
	if($PREF{internal_appname} eq 'userbase')
	{
		...
		my $input_username	= param($PREF{userbase_user_fieldname}) || exit_with_error(qq`$TEXT{empty_username_error}`);
		my $input_password	= param($PREF{userbase_pass_fieldname}) || exit_with_error(qq`$TEXT{empty_password_error}`);
		my $ref = param("ref");
		my ($expiry) = ();
...
elsif($qs =~ /action=$PREF{mkdir_action_name}(?:&path=(.+?)(?:&dirname=(.+))?)?(?:&|$)/)
{
	exit_with_error(qq`Disabled for trial version.`);
}
...
elsif($qs =~ /do_email_test/   &&   $PREF{enable_email_test} =~ /yes/i)
{
	do_email_test();
}
else
